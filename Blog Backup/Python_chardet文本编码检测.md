...
针对全是`ascii`表范围字符组成的文件, 例如英文符号组成的`utf-8`文件(无`bom`/`sig`), 由于ascii和utf-8, 处理这些字符均采用1为来存储, 其表示的方式是一样的.
...
C:\Users\Lian\anaconda3\lib\site-packages\charset_normalizer\api.py:105: UserWarning: Trying to detect encoding from a tiny portion of (12) byte(s).
